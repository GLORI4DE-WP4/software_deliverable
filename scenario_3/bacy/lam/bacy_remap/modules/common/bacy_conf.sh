...
case "$BA_SITE_HPC" in
  ...
  HOREKA_CPU)
    ...
    ;;	
  LEONARDO_GPU)
    f_call_set_var  BA_PATH        /leonardo_work/DE360_GLORI/smr_prod/scenarios/scenario_3/bacy/bacy_data
    ;;
esac
###################################################################################
#
...
case "$1" in                                                          # window size
  ...
  ICON-LAM)
    case "$BA_I_GNUM_DET" in
      0042)  f_call_set_var -t  BA_I_IAU_W_DELTA_DET  600s;;
      *)     f_call_set_var -t  BA_I_IAU_W_DELTA_DET  600s;;
    esac
    case "$BA_I_GNUM_ENS" in
      0042)  f_call_set_var -t  BA_I_IAU_W_DELTA_ENS  600s;;
...
# f_call_set_var  BA_I_IAU_W_SHIFT_DET 0
# f_call_set_var  BA_I_IAU_W_SHIFT_ENS 0
#----------------------------------------------------------------------------------
...
f_call_set_var  BA_L_BC_I_N_LEVEL   137                 # vertical levels of source
                                                        #  (domain 1)
#----------------------------------------------------------------------------------
f_call_set_var  BA_L_BC_SRC_FNAME   'IFS' # filename source: 'BACY','DBASE'
                                          #   DBASE: e.g. iefff....
                                          #   BACY: e.g. fc_....
                                          #   IFS: e.f. fc_ifs...
f_call_set_var  BA_L_BC_SRC_GLOBAL  0     # get boundary data from global data: 0|1
f_call_set_var  BA_L_BC_SRC_MODE    'SAME' # data source from -SAME:  same MODE
                                          #                   -ASS: ass  cycle
                                          #                  -MAIN: main cycle
#----------------------------------------------------------------------------------
