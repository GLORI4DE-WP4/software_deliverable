...
f_call_set_var  MO_I_MAP_FILE_BC     ${BA_PATH}/const/ana_varnames_map_file.txt
#----------------------------------------------------------------------------------
...
case "${MO_IN_MODEL}:${MO_IN_MODE}" in    # latent heat nudging: active|passive|off
  ...
  ICON-LAM:ASS)              f_call_set_var  MO_L_LHN   'off';;
  ICON-LAM:GEN_START_DATA)   f_call_set_var  MO_L_LHN   'off';;
...
case "${MO_IN_MODEL}:${MO_IN_MODE}" in   # run MAIN (ILAM) with interp. ICON-EU 0|1
  ICON-LAM:MAIN)
    f_call_set_var  MO_L_MAIN_FROM_IEU   0 #${EX_MO_L_MAIN_FROM_IEU:-0}
    ;;
esac
#----------------------------------------------------------------------------------
case "${MO_IN_MODEL}:${MO_IN_MODE}" in   # run MAIN (ILAM) with interp. IFS "IFS"|0
  ICON-LAM:MAIN)
    f_call_set_var  BA_L_BC_SRC_FNAME   "BACY"
    ;;
esac
###################################################################################
#
