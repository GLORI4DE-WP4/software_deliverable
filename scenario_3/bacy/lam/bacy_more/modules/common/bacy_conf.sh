...
case "$BA_SITE_HPC" in
  ...
  HOREKA_CPU)
    ...
    ;;	
  LEONARDO_GPU)
    f_call_set_var  BA_PATH        /leonardo_work/DE360_GLORI/smr_prod/scenarios/scenario_3_da/bacy_data
    ;;
esac
###################################################################################
#
...
case "$1" in
  COSMO)
    ...
    ;;
  ICON-LAM)
    f_call_set_var BA_CDFIN_FPATH     ${BA_PATH}/obs_local
    f_call_set_var BA_L_OBSDIR[CONV]  ${BA_PATH}/obs_local
    f_call_set_var BA_L_OBSDIR[LHN]   ${BA_L_OBSDIR[CONV]}/lhn
    f_call_set_var MO_L_LHN           'off'
    ;;
esac
...
#==================================================================================
f_call_set_var BA_MODES_IN_CONV            .FALSE.    # Da Marcello: 05/02/2024
#==================================================================================
...
#----------------------------------------------------------------------------------
f_call_set_var  BA_L_BC_GNUM_DET    9999    # ICON gridnumber boundary fields (det)
...
f_call_set_var  BA_L_BC_I_N_LEVEL   137                 # vertical levels of source
                                                        #  (domain 1)
#----------------------------------------------------------------------------------
f_call_set_var  BA_L_BC_SRC_FNAME   'BACY'    # filename source: 'BACY','DBASE'
                                               #   DBASE: e.g. iefff....
                                               #   BACY: e.g. fc_....
                                               #   IFS: e.f. fc_ifs...
...
#==================================================================================
f_call_set_var BA_ENSIZE_ASS       40  # ensemble size (for assimilation forecasts)
#----------------------------------------------------------------------------------
case "$1" in
  COSMO|ICON-LAM)
...
case "$1" in
  ...
  COSMO)
    ...
    ;;
  ICON-LAM)
    f_call_set_var -t BA_FCLNG_DET        48h                      # main fc length
    f_call_set_var -t BA_FCLNG_ENS        48h                      # main fc length
    f_call_set_var -t BA_FCSTP_DET         1h            # main fc output frequency
    f_call_set_var -t BA_FCSTP_ENS         1h            # main fc output frequency
    ;;
esac
...
#==================================================================================
f_call_set_var BA_LETKF_DET_RUN   0                                           # 0|1
###################################################################################
#
#
###################################################################################
