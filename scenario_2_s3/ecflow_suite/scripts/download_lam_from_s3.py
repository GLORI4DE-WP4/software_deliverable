...
def download_files_from_s3(bucket_name, destination, fc_date="unknown",
    ...
    # Formatting the expected file prefixes
    expected_prefixes = []
    for to_format in LAM_TEMPLATE_DICT['date_str']:
        expected_prefixes.append(to_format.format(fc_date))
    for to_format in LAM_TEMPLATE_DICT['no_format']:
        expected_prefixes.append(to_format)

    # Select items to download
    count=0
    objects_to_download = []
    for res in response['Contents']:
        for possible_prefix in expected_prefixes:
            if res['Key'].startswith(possible_prefix):
