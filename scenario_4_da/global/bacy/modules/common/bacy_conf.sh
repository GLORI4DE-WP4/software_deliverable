...
case "$BA_SITE_HPC" in
  ...
  HOREKA_CPU)
    ...
    ;;	
  LEONARDO_GPU)
    f_call_set_var  BA_PATH        /leonardo_work/DE360_GLORI/smr_prod/scenarios/scenario_4_da/bacy_data
    f_call_set_var  BA_FDB_PATH    ${BA_PATH}/fdb_output
    ;;
esac
###################################################################################
#
...
#----------------------------------------------------------------------------------
f_call_set_var  BA_ICON_GRIDDIR   ${BA_ROUTFOXFOX}/icon/grids
...
case "$1" in                                                          # grid number
  ICON)
    f_call_set_var  BA_I_GNUM_DET   0036                     # ICON (deterministic)
...
case "$1" in                                                          # window size
  ICON)
    case "$BA_I_GNUM_DET" in
      ...
      *)     f_call_set_var -t  BA_I_IAU_W_DELTA_DET  10560s;;
    esac
  ...
  ICON-LAM)
    case "$BA_I_GNUM_DET" in
      ...
      *)     f_call_set_var -t  BA_I_IAU_W_DELTA_DET  600s;;
    esac
    case "$BA_I_GNUM_ENS" in
...
case "$1" in
  ...
  ICON-LAM) f_call_set_var  BA_I_NEST  0;;
esac
#==================================================================================
fi
###################################################################################
...
f_call_set_var BA_L_USE[RADWINDVOL]        7          #          2 (online passive)
f_call_set_var BA_L_USE[RADREFLVOL]        7          #          3 (online monitor)
...
f_call_set_var BA_L_USE[SEVIRI_VIS]        7
f_call_set_var BA_L_USE[SEVIRI_WV]         7
...
f_call_set_var  BA_L_BC_SRC_MODE    'ASS'      # data source from -SAME:  same MODE
                                               #                   -ASS: ass  cycle
                                               #                  -MAIN: main cycle
#----------------------------------------------------------------------------------
...
#==================================================================================
f_call_set_var BA_ENSIZE_ASS       4  # ensemble size (for assimilation forecasts)
#----------------------------------------------------------------------------------
case "$1" in
  COSMO|ICON-LAM)
    f_call_set_var BA_ENSIZE_MAIN      4      # ensemble size (for main forecasts)
    ;;
  ICON)
    f_call_set_var BA_ENSIZE_MAIN      4      # ensemble size (for main forecasts)
    ;;
...
#----------------------------------------------------------------------------------
f_call_set_var BA_I_OBSDIR_CONV[0]  $BA_I_OBSDIR
...
#----------------------------------------------------------------------------------
f_call_set_var BA_I_OBSDIR_SATPP ""
...
case "$1" in
  ICON)
    f_call_set_var -t BA_FCLNG_DET         4h                      # main fc length
    f_call_set_var -t BA_FCLNG_ENS         4h                      # main fc length
    f_call_set_var -t BA_FCSTP_DET         2h            # main fc output frequency
    f_call_set_var -t BA_FCSTP_ENS         2h            # main fc output frequency
    ;;
  COSMO)
