...
case "$BA_SITE_HPC" in
  ...
  HOREKA_CPU)
    ...
    ;;	
  LEONARDO_GPU)
    f_call_set_var  BA_PATH        /leonardo_work/DE360_GLORI/smr_prod/scenarios/scenario_4_s3/bacy_data
...
case "$1" in                                                          # window size
  ICON)
    case "$BA_I_GNUM_DET" in
      0036)  f_call_set_var -t  BA_I_IAU_W_DELTA_DET  0s;; # 10560s;;
      *)     f_call_set_var -t  BA_I_IAU_W_DELTA_DET  0s;; #10800s;;
    esac
...
case "$1" in
  ICON)
    f_call_set_var -t BA_FCLNG_DET         2h                      # main fc length
    f_call_set_var -t BA_FCLNG_ENS         2h                      # main fc length
