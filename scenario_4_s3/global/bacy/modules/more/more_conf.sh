...
#----------------------------------------------------------------------------------
f_call_set_var  MO_I_INIT_MODE_DET    7
...
case "$MO_IN_MODEL" in
  ICON)
    ...
    f_call_set_var  MO_YACO     /leonardo_work/DE360_GLORI/mgisonni/yaco/scripts/yaco_nvhpc_yac3.2.0/build/yaco-main/yaco
    ;;
  ICON-LAM)
    f_call_set_var  MO_BINARY   ${BA_PATH}/bin/icon
    f_call_set_var  MO_WRAPPER  ${BA_PATH}/bin/wrapper
...
case "${MO_IN_MODEL}:${MO_IN_MODE}" in    # latent heat nudging: active|passive|off
  ...
  ICON-LAM:MAIN)    f_call_set_var  MO_L_LHN   'active';;
  ICON-LAM:*)       f_call_set_var  MO_L_LHN   'active';;
esac
#----------------------------------------------------------------------------------
case "${MO_IN_MODEL}:${MO_IN_MODE}" in   # run MAIN (ILAM) with interp. ICON-EU 0|1
  ICON-LAM:MAIN)
    f_call_set_var  MO_L_MAIN_FROM_IEU   ${EX_MO_L_MAIN_FROM_IEU:-0};;
esac
###################################################################################
#
#
